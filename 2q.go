...
// parameters.
type TwoQueueCache[Key, Value any] struct {
	size       int
	recentSize int

	recent      simplelru.LRUCache[Key, Value]
	frequent    simplelru.LRUCache[Key, Value]
	recentEvict simplelru.LRUCache[Key, struct{}]
	lock        sync.RWMutex
}
...
func New2Q[Key comparable, Value any](size int) (*TwoQueueCache[Key, Value], error) {
	return New2QParams[Key, Value](size, Default2QRecentRatio, Default2QGhostEntries)
}
...
func New2QParams[Key comparable, Value any](size int, recentRatio, ghostRatio float64) (*TwoQueueCache[Key, Value], error) {
	...
	// Allocate the LRUs
	recent, err := simplelru.NewLRU[Key, Value](size, nil)
	if err != nil {
		return nil, err
	}
	frequent, err := simplelru.NewLRU[Key, Value](size, nil)
	if err != nil {
		return nil, err
	}
	recentEvict, err := simplelru.NewLRU[Key, struct{}](evictSize, nil)
	if err != nil {
		return nil, err
	}

	// Initialize the cache
	c := &TwoQueueCache[Key, Value]{
		size:        size,
...
func (c *TwoQueueCache[Key, Value]) ensureSpace(recentEvict bool) {
